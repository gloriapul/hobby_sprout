...
content_id: 2cbe960c97b41c6c4cdbeba487dc0c712b41ed079ae1696627bc65bb1278c54b
---
...
export default class MilestoneTrackerConcept {
   ...
   * including its `id`, `description`, and `isActive` status.
   */
  async _getGoal({
    user,
  }: {
   ...
   * including their `id`, `description`, and `start` date.
   */
   ...
   * including their `id`, `description`, `start` date, and `completion` date.
   */
  async _getCompleteSteps({
    ...
    const stepsDocs = await this.steps
      .find({ goalId: goal, isComplete: true, completion: { $exists: true } })
      .toArray();
    return stepsDocs.map((s) => ({
      id: s._id,
      description: s.description,
      start: s.start,
      completion: s.completion!, // Assert non-null because of the $exists check
    }));
  }
}
```
