...
content_id: e1acbaabf24638aaf9b929d9ed7903821c618953fc868ddb7c56a691ff57741b
---
...
export default class MilestoneTrackerConcept {
  ...
  // Constructor now takes the Gemini API key
  ...
  async generateSteps({
    ...
    try {
      ...
      // Call the LLM to get step descriptions
  ...
  async completeStep({ step }: { step: Step }): Promise<Empty | { error: string }> {
    ...
    // Check if the associated goal is active
  ...
  async _getCompleteSteps({
    ...
    return stepsDocs.filter(s => s.completion !== undefined).map((s) => ({ // Filter for completed steps with a completion date
      ...
      description: s.description,
      start: s.start,
      completion: s.completion!, // Assert non-null after filtering for typesafety
    }));
  }
}
```
