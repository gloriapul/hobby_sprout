...
content_id: bbc45d47c1bbf5be572a54c9e1f525596a45c52a86024b498988be06dc8cfd22
---
...
import { GeminiLLM } from "@utils/gemini-llm.ts"; // Import GeminiLLM
...
export default class MilestoneTrackerConcept {
  ...
  async createGoal({
    ...
    try {
      ...
      console.log(`Goal created for user ${user}: ${newGoalId}`);
  ...
  async generateSteps({
    ...
    try {
      ...
      console.log(`Generated ${newStepDocs.length} steps for goal ${goal}.`);
  ...
  async addStep({
    ...
    try {
      ...
      console.log(`Step added to goal ${goal}: ${newStepId}`);
  ...
  async _getCompleteSteps({
    ...
    return stepsDocs.filter(s => s.completion !== undefined).map((s) => ({
      id: s._id,
      description: s.description,
      start: s.start,
      completion: s.completion!,
